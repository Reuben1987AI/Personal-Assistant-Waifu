...
**Deferred.** The rolling-buffer design (livekit-wakeword's intended architecture) is implemented, compiles clean, and *does* fire on a spoken "Kassandra" — but live latency is ~16s and CPU-saturating, making it unusable as a real-time trigger. We are **moving to manual call start** (UI button → Qwen call, no wake word) until the inference-speed problem is solved. See "Next steps" for the path back to automatic wake.
...
**The math:**
- tract ONNX predict: **~1.6s** per 2s-window call (mel + embedding + classifier)
- model scoring window: **~0.5s** (position band where score > threshold)
- therefore: predict interval (1.6s) > scoring window (0.5s)

A non-blocking/threaded rolling buffer (the "obvious" fix for the 16s lag) would make this *worse*, not better: the buffer would slide in real-time, the word would pass through the 0.5s scoring window in 0.5s, and a predict would land in that window only ~31% of the time (0.5 / 1.6). The current blocking design accidentally slows the slide so the word dwells in the scoring window across multiple predicts — that's why it works at all, but with 16s latency and a pinned core.

The rolling-buffer continuous-predict design is **architecturally correct only when predict ≪ scoring window**. With tract at 1.6s/predict and a 0.5s window, that condition is violated. This is the single fact that blocks shipping automatic wake.
...
3. **livekit-wakeword pulls `ort` with the `alternative-backend` feature**, which disables ONNX Runtime linking entirely and forces everything through `ort-tract` (pure-Rust inference). Tract is ~1.6s/predict on this model; real onnxruntime is 10–50× faster. This is the root cause of the latency. There is no config flag to flip — the constraint is baked into livekit-wakeword's dependency.
4. **Silero VAD is not usable** in any project that depends on livekit-wakeword (tract can't translate its `If` nodes). Tried 3 model variants — all failed. livekit's Python `livekit-plugins-silero` uses real onnxruntime and isn't portable to Rust; no equivalent Rust VAD crate exists in livekit/rust-sdks.
5. **The intended production design for livekit-wakeword is rolling-buffer continuous predict.** The mel+embedding pipeline is itself the "is this speech" filter — non-speech scores ~0.005. Framing crutches (lead silence, end-of-utterance detection, VAD gating) were workarounds for an undertrained, position-sensitive model. The rolling design eliminates them all — *assuming a fast classifier*.
6. **AEC noise suppression (NS Moderate) does not hurt the model.** Live mic peaks at 0.53 vs clean sample 0.38 — NS-processed mic is fully recognizable. Don't disable NS chasing accuracy; it's not the problem.
7. **The model is undertrained.** Peaks at 0.38 on clean TTS-like audio, 0.53 on live mic. Configured `wakeword-configs/kassandra.yaml` calls for `medium` / 150k steps / 30k samples; the current model was trained with less. Wider training → wider scoring window + higher peak, which helps but doesn't fix the 1.6s predict.
8. **`webrtc-vad`'s `Vad` struct holds a raw `*mut Fvad` (not `Send`)** — needed `unsafe impl Send for Vad` to cross the tokio boundary. Module deleted; kept here as a record.
9. **curl on GitHub `/raw/refs/heads/master/...` URLs returns 404 for some paths** — silero VAD models moved from `files/` to `src/silero_vad/data/`. Always verify `file <downloaded>` is `data` not `HTML document` before trusting a model download.
...
### Path B — replace tract with real onnxruntime (the real latency fix)
The 1.6s/predict is because livekit-wakeword forces `ort-tract`. Real `onnxruntime` (via `ort` with `download-binaries`) is 10–50× faster — predict drops to ~30–50ms, well under the 0.5s scoring window, and pure rolling-buffer continuous predict becomes reliable. Two routes:
- **B1:** Fork livekit-wakeword, swap its `ort` features from `alternative-backend` to `download-binaries`. Risk: it may depend on tract-specific behavior; the mel/embedding models are bundled and may not load under real onnxruntime without tweaks.
- **B2:** Reimplement the mel + embedding + classifier pipeline ourselves on top of `ort` with `download-binaries`. The three ONNX models are extractable from livekit-wakeword. More work but no upstream fight.

This is days of work, not hours. Defer until manual call start (Path A) is shipping and the app is otherwise usable.

### Path C — retrain the model (accuracy, not latency)
`medium` / 150k steps / 30k samples per `wakeword-configs/kassandra.yaml`. Wider scoring window + higher peak → more forgiving of the 1.6s predict interval (a 1.5s scoring window would make the rolling design hit ~94% of the time even at 1.6s cadence). Needs `HF_TOKEN` + compute. Doesn't make predict faster. Do this alongside or after Path B.

### Path D — utterance-gated predict on the rolling buffer (pragomatic fallback)
Keep the rolling buffer as the *source* of the predict window, but instead of continuous predict, predict **once** when the energy gate goes inactive (800ms after the user stops speaking). At that moment the word has slid to ~the scoring position (you finished ~0.8s ago, word now near the front of the 2s buffer). ~1 predict per utterance → no CPU saturation, deterministic hit, ~2.5s latency (0.8s silence confirm + 1.6s predict). **This is the utterance-framing crutch we just removed** — but it's the crutch that actually works with a 1.6s-predict, 0.5s-window model. Consider only if Path B stalls and we need automatic wake before retraining.
